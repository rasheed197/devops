...
#### Step 1
open the file /etc/sudoers in write mode in vim editor
```
visudo /etc/sudoers
```

This file has no write permission so we can only edit it this way
```
...
You can also add the username there instead of group, e.g to add an "aws" user
