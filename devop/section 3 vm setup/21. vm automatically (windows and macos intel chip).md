...
1. No OS installation
    - vagrant set up VM machines by using VMs images, or in vagrant, they're called BOX
    - These are ready made VMs stored on vagrant cloud, which vagrant will download and keep it on your local computer
    - From that box, you can create as many VM as you want

2. All the VM settings will be in a file called "Vagrantfile"
    - You can make changes to the VM settings like RAM, CPU, IP addresses all through this vagrantfile
    - you can also do PROVISIONING with vagrant
    - Provisioning generally means when the OS is completely booted, you want to execute some commands on it.
...
# check status of all VMs as well VMs ID
...
# delete the VM (you have to be in the VM directory)
vagrant destroy

# delete the VM (from any directory)
...
- vagrant is going to read the vagrantfile and check if the box i.e the OS image is available in your local machine
...
```
- The "Vagrantfile" file will be placed in your current directory. To confirm do
```
$ ls
```
